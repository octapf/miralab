...
export async function generateMetadata({ params }: DeleteAccountPageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'matchpointDeleteAccount' });
...
export default async function MatchpointDeleteAccountPage({ params }: DeleteAccountPageProps) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'matchpointDeleteAccount' });
  ...
  return (
    <main style={sectionStyle}>
      ...
      <div style={actionRowStyle}>
        ...
        <Link href={`/${locale}/matchpoint/privacy`} style={secondaryActionStyle}>
          {t('privacyCta')}
        </Link>
      </div>
