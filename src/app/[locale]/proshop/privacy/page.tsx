...
export async function generateMetadata({ params }: PrivacyPageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'proshopPrivacy' });
...
export default async function ProshopPrivacyPage({ params }: PrivacyPageProps) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'proshopPrivacy' });

  return (
    <main style={sectionStyle}>
      <Link href={`/${locale}/proshop`} style={backButtonStyle}>
        {t('backButton')}
      </Link>
