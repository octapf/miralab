...
export async function generateMetadata({ params }: DeleteAccountPageProps): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'rizeDeleteAccount' });
...
export default async function RizeDeleteAccountPage({ params }: DeleteAccountPageProps) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'rizeDeleteAccount' });

  return (
    <main style={sectionStyle}>
      <Link href={`/${locale}/rize`} style={buttonStyle}>
        {t('backButton')}
      </Link>
      ...
      <div style={actionRowStyle}>
        ...
        <Link href={`/${locale}/rize/privacy`} style={secondaryActionStyle}>
          {t('privacyCta')}
        </Link>
      </div>
